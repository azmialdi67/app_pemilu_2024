...
class M_calon extends CI_Model
{
	...
	private function _get_datatables_query()
	{
		...
		if ($this->session->userdata('id_role') == '3') {
			...
			$this->db->or_where('tbl_calon.kdkec', 'all');
		}
	...
	public function save($data)
	{
		// Handle pemilihan "Semua Desa" atau "Semua Kecamatan"
		if ($data['kdkec'] == 'all') {
			// Set nilai sesuai kebutuhan untuk semua kecamatan
			$data['kdkec'] = 'all';
		}

		if ($data['kddesa'] == 'all') {
			// Set nilai sesuai kebutuhan untuk semua desa
			$data['kddesa'] = 'all';
		}
	...
	{
		if ($this->session->userdata('id_role') == '3') {
			$sql = "SELECT COUNT(*) AS jmlcalon, SUM(s_hasil) AS jmlsuara FROM tbl_calon WHERE kdkec LIKE {$kc} AND thn_pemilihan = " . $this->session->userdata('thn_data') . " OR kdkec = 'all'";
		} else {
	...
	function load_data($ds)
	{
		...
		$this->db->where('kddesa', $ds)->or_where('kddesa', 'all');
		$this->db->where('tbl_calon.thn_pemilihan', $this->session->userdata('thn_data'));
