...
class M_penyelenggara extends CI_Model
{
	...
	var $column_order = array(null, 'tbl_wdesa.nama_desa', 'tbl_wkecamatan.nama_kec', 'tbl_desa_penyelenggara.dpt_l', 'tbl_desa_penyelenggara.dpt_p', null, 'tbl_desa_penyelenggara.suratsuara', null, 'tbl_tps.nama_tps'); //set column field database for datatable orderable
	...
	private function _get_datatables_query()
	{
		...
		$this->db->join('tbl_tps', 'tbl_tps.id_tps=tbl_desa_penyelenggara.kdtps', 'left');
		...
		if ($this->input->post('nama_tps')) {
			$this->db->like('tbl_tps.nama_tps', $this->input->post('nama_tps'));
		}
	...
	{
		$this->db->select('tbl_wkecamatan.nama_kec,tbl_wdesa.nama_desa,tbl_desa_penyelenggara.dpt_l,tbl_desa_penyelenggara.dpt_p,tbl_desa_penyelenggara.suratsuara,tbl_tps.nama_tps');
		$this->db->join('tbl_wkecamatan', 'tbl_wkecamatan.id_kec=tbl_desa_penyelenggara.kdkec', 'left');
		$this->db->join('tbl_wdesa', 'tbl_wdesa.id_desa=tbl_desa_penyelenggara.kddesa', 'left');
		$this->db->join('tbl_tps', 'tbl_tps.id_tps=tbl_desa_penyelenggara.tps', 'left');
		//filter tahun penlaksanaan
		$this->db->where('tbl_desa_penyelenggara.thn_pemilihan', $this->session->userdata('thn_data'));
	...
	public function select_by_kategori()
	{
		if ($this->session->userdata('id_role') == 3) {
			...
			LEFT JOIN tbl_tps ON tbl_tps.id_tps = tbl_desa_penyelenggara.kdtps
	...
	public function select_jml_tps()
	{
		if ($this->session->userdata('id_role') == '3') {
			$sql = "SELECT COUNT(DISTINCT(kdtps)) AS jmltps FROM tbl_desa_penyelenggara WHERE thn_pemilihan = '" . $this->session->userdata('thn_data') . "' AND kdtps = '" . $this->session->userdata('id_desa') . "'";
		} else {
			$sql = "SELECT COUNT(DISTINCT(kdtps)) AS jmldesa FROM tbl_desa_penyelenggara WHERE thn_pemilihan = '" . $this->session->userdata('thn_data') . "'";
		}
	...
	{
		$this->db->select('tbl_desa_penyelenggara.kdtps,tbl_tps.id_tps');
		$this->db->join('tbl_tps', 'tbl_tps.id_tps=tbl_desa_penyelenggara.kdtps', 'left');
		...
		$this->db->from($this->table);
		$this->db->order_by('tbl_tps.id_tps', 'asc');
		$query = $this->db->get();
		$result = $query->result();
