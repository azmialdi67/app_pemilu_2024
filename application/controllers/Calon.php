...
class Calon extends AUTH_Controller
{
	...
	public function index()
	{
		...
		$data['deskripsi'] 		= "Data Calon Presiden dan Wakil Presiden Tahun " . $this->session->userdata('thn_data');
	...
	// 	// Jika admin memilih "Semua Kecamatan" dan "Semua Desa"
	// 	if ($data['kdkec'] == 'all' && $data['kddesa'] == 'all') {
	// 		// Simpan data ke seluruh kecamatan dan desa
	// 		$this->model_anda->insertDataForAllKecamatanDanDesa($data);
	// 	} else {
	// 		// Simpan data sesuai dengan kecamatan dan desa yang dipilih
	// 		$this->model_anda->insertData($data);
	// 	}
	...
	{
		if ($id_kec == 'all') {
			// Jika id_kec adalah 'all', maka pilih seluruh kecamatan
			$data = "<option value='all'>Seluruh Kecamatan</option>";

			// Ambil semua data kecamatan
			$kecamatanQuery = $this->db->get('tbl_wkecamatan');
			foreach ($kecamatanQuery->result() as $kecamatan) {
				$data .= "<option value='" . $kecamatan->id_kec . "'>" . $kecamatan->nama_kec . "</option>";
			}
		} else {
			// Jika id_kec bukan 'all', ambil data desa berdasarkan kecamatan
			$query = $this->db->get_where('tbl_wdesa', array('kecamatan_id' => $id_kec));
			$data = "<option value='all'>Semua Desa</option>";
