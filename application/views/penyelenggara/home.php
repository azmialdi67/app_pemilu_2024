		...
		<div class="panel panel-primary">
			<div class="panel-body">
				<div class="col-md-6 col-xs-12">
					<?php
					echo '<h4 class="text-muted"><b>Data Penyelenggaraan pilpres Serentak Tahun ' . $this->session->userdata('thn_data') . '</b></h4>';
					?>
				</div>
	...
	$(document).ready(function() {
		...
		table = $('#table').DataTable({
			"footerCallback": function(row, data, start, end, display) {
				...
				var dptlTotal = api.column(3).data().reduce(function(a, b) {
					return intVal(a) + intVal(b);
				}, 0);



				var dptpTotal = api.column(4).data().reduce(function(a, b) {
					return intVal(a) + intVal(b);
				}, 0);

				var jmlTotal = api.column(5).data().reduce(function(a, b) {
					return intVal(a) + intVal(b);
				}, 0);

				var suratTotal = api.column(6).data().reduce(function(a, b) {
					return intVal(a) + intVal(b);
				}, 0);

				// Update footer by showing the total with the reference of the column index 
				$(api.column(0).footer()).html('Total');
				$(api.column(2).footer()).html(input4.value);
		...
		var input4 = document.getElementById('tps_data');
		...
		input4.addEventListener('change', function() {
			input3.value = parseInt(input1.value) + parseInt(input2.value) + parseInt(input4.value);
			//input4.value = parseInt(input3.value) + parseInt((input3.value * 2.5) / 100);
		});
				...
				<form action="#" id="form" class="form-horizontal">
					...
					<div class="form-body">
						...
						<div class="form-group">
							...
							<div class="col-md-9">
								<select id="tps_data" name="kdtps" class="form-control">
									<option>-- Pilih Tps --</option>
									<?php
									foreach ($datatpsnya as $tps_data) {
										echo "<option value='" . $tps_data->id_tps . "'>" . $tps_data->nama_tps . "</option>";
									}
									?>
								</select>
								<span class="help-block"></span>
					...
					<div class="form-group">
						...
						<div class="col-md-9">
							<input name="ketua" placeholder="Ketua Panitia pilpres" class="form-control" type="text">
							<span class="help-block"></span>
						</div>
					</div>
			</div>
