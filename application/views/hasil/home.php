...
<script type="text/javascript">
    ...
    function load_nama_calon(kode) {
        $.ajax({
            url: "<?php echo base_url(); ?>hasil/load_data/" + kode + "?kdkec=all",
            dataType: "JSON",
            success: function(data) {
                var html = '';
                ...
                for (var count = 0; count < data.length; count++) {
                    html += '<tr>';
                    html += '<td>' + data[count].nama + '</td>'; // Menampilkan nama calon
                    html += '<td><input id="angka_' + data[count].id + '" type="number" min="0" step="1" style="width: 80px" class="table_data pull-right" data-row_id="' + data[count].id + '" data-column_name="s_hasil" value="' + data[count].s_hasil + '"></td>'; // Inputan angka untuk perolehan suara
                    html += '</tr>';
                }
    ...
    function edit_person(id) {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                $('.modal-title').text('Input Hasil pilpres'); // Set title to Bootstrap modal title
